...
class CusomerRefData(Resource):
    ...
    def execute_db(data):
        ...
        try:
            ...
            sql = 'SELECT COUNT(*) FROM OSSRPT.GPS_CUS_LOC_180 WHERE CRM_REF_ID = :CRM_REF_ID'
            cursor.execute(sql, {'CRM_REF_ID': data['OID']})
            srchrec = cursor.fetchone()[0]

            if srchrec == 1:

                sql2 = 'SELECT CUS_REF_ID FROM GPS_CUS_LOC_180 WHERE CRM_REF_ID = :CRM_REF_ID'
                cursor.execute(sql2, {'CRM_REF_ID': data['OID']})
                CUS_REF_ID = cursor.fetchone()[0]

                result['msg'] = "Success"
                result['data'] = CUS_REF_ID

            else:
                result['msg'] = "Error"
                result['data'] = "Relavent Data Not Found"
...
class GetCusRefData(Resource):
    ...
    def post():
        ...
        if not data:
            return {"msg": "No data provided"}, 400
        ref = random_ref(15)
        return CusomerRefData.execute_db(data)

api.add_resource(AddCustomerData, '/ApiCusGps/addcusData/')

api.add_resource(GetCusRefData, '/ApiCusGps/getcusrefData/')
